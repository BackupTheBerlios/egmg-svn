                           ...
                           const NumericArray& opL )
{
    ...
    m_coefficients[ level ][ sy*nx+sx ].resize( opL.size() );
    m_coefficients[ level ][ sy*nx+sx ] = opL;
}
void StencilBoard::insert( const Index level,
             const Position pos,
             ...
             const PositionArray& jY )
{
    ...
    m_posJx[ level ][ pos ].resize( jX.size() );
    m_posJy[ level ][ pos ].resize( jY.size() );
    m_posJx[ level ][ pos ] = jX;
    m_posJy[ level ][ pos ] = jY;
}
                         ...
                         const Index ny ) const
{
    if ( m_coefficients.size() < level )
        return false;
    if ( m_coefficients[ level ].size() != nx*ny )
        return false;
    if ( m_coefficients[ level ][ sy*nx+sx ].size() == 0 )
        return false;
    return true;
}
bool StencilBoard::find( const Index level,
                         const Position pos ) const
{
    if ( m_posJx.size() < level || m_posJy.size() < level  )
        return false;
    if ( m_posJx[ level ][ pos ].size() == 0 || 
         m_posJy[ level ][ pos ].size() == 0 )
        return false;
    return true;
}
...
{
    //ASSERT( level < m_coefficients.size() )
    //ASSERT( m_coefficients[ level ].size() == nx*ny )
    //ASSERT( m_coefficients[ level ][ sy*nx+sx ].size() > 0 )
    return m_coefficients[ level ][ sy*nx+sx ];
}
const PositionArray& StencilBoard::getJx( const Index level,
                                          const Position pos ) const
{
    //ASSERT( level <= m_posJx.size() && m_posJx.size() == m_posJy.size() )
    //ASSERT( m_posJx[ level ][ pos ].size() == m_posJy[ level ][ pos ].size() )
    //ASSERT( m_posJx[ level ][ pos ].size() > 0 )
    return m_posJx[ level ][ pos ];
}
const PositionArray& StencilBoard::getJy( const Index level,
                                          const Position pos ) const
{
    //ASSERT( level <= m_posJy.size() && m_posJx.size() == m_posJy.size() )
    //ASSERT( m_posJx[ level ][ pos ].size() == m_posJy[ level ][ pos ].size() )
    //ASSERT( m_posJy[ level ][ pos ].size() > 0 )
    return m_posJy[ level ][ pos ];
}
