namespace mg
{
	void lineGS::ninepointxzebra(std::valarray<Precision> &u, const std::valarray<Precision> &fv, 
    ...
    { 
		...
		if(stencil.isConstant() == true)
		{
			...
			for(size_t i=2; i<Ny ; i+=2)
			{
				...
				rhs[(Nx-1)+i*(Nx+1)] = fv[Nx-1+i*(Nx+1)] - L[n] * u[Nx-1+(i+J_y[n])*(Nx+1)] - L[s] * u[Nx-1+(i+J_y[s])*(Nx+1)] 
					- L[e] * u[Nx+i*(Nx+1)];
		...
		else
		{
			...
			if(Nx > 2)
			{
				...
				rhs[(Nx-1)+(Nx+1)] = fv[Nx-1+(Nx+1)] - L[n] * u[Nx-1+(1+J_y[n])*(Nx+1)] - L[s] * u[Nx-1+(1+J_y[s])*(Nx+1)] 
						- L[e] * u[Nx+(Nx+1)];
				for(size_t sum=5; sum<L.size(); sum++)
				{
				...
				rhs[(Nx-1)+(Ny-1)*(Nx+1)] = fv[Nx-1+(Ny-1)*(Nx+1)] - L[n] * u[Nx-1+(Ny-1+J_y[n])*(Nx+1)] - L[s] * u[Nx-1+(Ny-1+J_y[s])*(Nx+1)] 
				 	- L[e] * u[Nx+(Ny-1)*(Nx+1)];
				for(size_t sum=5; sum<L.size(); sum++)
				{
