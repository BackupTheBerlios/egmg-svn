/** \file TranspProlong.cpp
 * \author Benedikt Engbroks
 * \brief Contains the implementation of the class TranspProlong.
 */
...
NumericArray TranspProlong::restriction(
    ...
    const Index nx,const Index ny) const
{
	...
	for (Index sy=1;sy<nyNew+1;sy++)
	{
		for(Index sx=1;sx<nxNew+1;sx++)
		{
			...
			{
				if (sx == 1) pos = SW;
				else if (sx == nx-1) pos = SE;
				else pos = S;
			}
			else if (sy == ny-1)
			{
				if (sx == 1) pos = NW;
				else if (sx == nx-1) pos = NE;
				else pos = N;
			}
			else
			{
				if (sx == 1) pos = W;
				else if (sx == nx-1) pos = E;
				else pos = C;
			}
